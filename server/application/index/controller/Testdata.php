...
class Testdata extends Controller
{
    ...
    public function index()
    {
        ...
        $ret = $this->model->getTestdata($data);
        if ($ret) {
            return msg(200, $ret);
        } else {
    ...
    {
        $id = $this->param['id'];
        $ret = $this->model->getTestdataById($id);
        if ($ret) {
            return msg(200, $ret);
        } else {
    ...
    {
        $ret = $this->model->saveTestdata($this->param);
        if ($ret) {
    ...
    public function update()
    {
        ...
        $ret = $this->model->updateTestdata($id, $this->param);
        if ($ret) {
    ...
    public function enable()
    {
        ...
        $ret = $this->model->updateTestdata($id, $this->param, false);
        if ($ret) {
